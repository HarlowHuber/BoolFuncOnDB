...
bool get_dbFunc(std::fstream& dataset)
{
	...
	else
	{
		...
		for (int i = 0; i < (int)function.size(); i++)
		{
			...
			else if (function[i] == ' ' || function[i] == '&' || function[i] == '(' || function[i] == ')')
			{
				continue;
			}
			else if (function[i] == '>')
	...
	for (int i = 0; i < (int)thresholds.size(); i++)
	{
		if (kv_used)
		{
			...
			if (kv_thresholds[i].size() > 0)
			{
				...
				{
					results << " intervals: [-infinity;";
				}
				else
				{
					results << " intervals: [" << thresholds[i].first << ";";
				}
				...
				if (i < kv_thresholds.size())
				{
					...
					{
						results << kv_thresholds[i][j] << "] = " << j << "; (" << kv_thresholds[i][j] << "; ";
					}
				}

				if (thresholds[i].second == INT_MAX)
				{
					results << "infinity] = " << kv_thresholds[i].size() << "\n";
				}
				else
				{
					results << thresholds[i].second << "] = " << kv_thresholds[i].size() << "\n";
				}
			}
			else
			{
					...
					if (kv_thresholds[i].size() > 0)
					{
						...
						for (int j = 0; j < kv_attributes[i]; j++)
						{
							...
							if (j == 0)
							{
								...
								if (bound <= num && num <= kv_thresholds[i][j]) // first interval is []
								{
									datapoint[i] = j;
									break;
								}
							...
							else if (j == kv_attributes[i] - 1)
							{
								...
								if (kv_thresholds[i][j - 1] < num && num <= bound) // subsequent intervals are (]
								{
									datapoint[i] = j;
									break;
								}
